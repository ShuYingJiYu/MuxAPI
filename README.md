...
- **渠道级探测（可选）**：上游可开「渠道级探测」开关——探任一模型成功即视整渠道可用、自动复活该上游下所有未单独熔断的模型；运行时列只看渠道整体状态，模型徽章异常才单独亮出。上游对外状态按模型聚合：全部模型熔断（如只配一个探测且探测失败）则整渠道如实显示熔断
...
| `MUXAPI_MAX_RETRIES` | `3` | 单次请求最大换源重试次数 |
