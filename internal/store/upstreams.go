...
func (s *Store) ListEnabledByGroup(groupID int64) ([]*upstream.Upstream, error) {
	rows, err := s.db.Query(`SELECT u.id,u.name,u.source,u.base_url,u.api_key,u.proxy,u.protocol,u.billing_type,u.cache_mode,u.enabled,gu.priority,gu.weight,u.channel_probe,u.credit_ratio
...
func (s *Store) List() ([]*upstream.Upstream, error) {
	rows, err := s.db.Query(`SELECT id,name,source,base_url,api_key,proxy,protocol,billing_type,cache_mode,enabled,0,0,channel_probe,credit_ratio FROM upstreams ORDER BY sort_order, id`)
	if err != nil {
		return nil, err
	}
...
func (s *Store) Get(id int64) (*upstream.Upstream, error) {
	u := &upstream.Upstream{}
	err := s.db.QueryRow(`SELECT id,name,source,base_url,api_key,proxy,protocol,billing_type,cache_mode,enabled,channel_probe,credit_ratio FROM upstreams WHERE id=?`, id).
